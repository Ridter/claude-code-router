...
async function sendRequestToProvider(
  ...
  const maxRetries = fastify._server!.providerService.getApiKeyCount(provider.name);
  let lastError: any = null;
  let excludeKeyIndex = -1;

  // 尝试所有可用的 API Key
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      // 选择一个 API Key
      const { key: selectedApiKey, index: keyIndex } = fastify._server!.providerService.selectApiKey(
        provider.name,
        excludeKeyIndex
      );

      // 记录使用的 key 索引（脱敏）
      fastify.log.debug(`Using API key index ${keyIndex} for provider ${provider.name} (attempt ${attempt + 1}/${maxRetries})`);

      // 保存当前使用的 key 索引到 context
      context.currentApiKeyIndex = keyIndex;
      ...
      if (bypass && typeof transformer.auth === "function") {
        // 创建临时 provider 对象，使用选中的 key
        const tempProvider = { ...provider, apiKey: [selectedApiKey] };
        const auth = await transformer.auth(modifiedRequestBody, tempProvider, context);

        if (auth.body) {
          modifiedRequestBody = auth.body;
          ...
          modifiedRequestBody = auth;
        }
      }

      // 发送HTTP请求，使用选中的 API Key
      ...
      if (!response.ok) {
        ...
        // 如果是限流或认证错误，尝试下一个 key
        if (status === 429 || status === 401 || status === 403) {
          fastify.log.warn(
            `API key index ${keyIndex} failed with status ${status}, trying next key...`
          );
          excludeKeyIndex = keyIndex;
          lastError = createApiError(
            `Error from provider(${provider.name},${modifiedRequestBody.model}): ${status}: ${errorText}`,
            status,
      ...
      // 如果是网络错误或其他异常，记录并尝试下一个 key
      if (attempt < maxRetries - 1) {
        fastify.log.warn(
          `Request failed with error: ${error.message}, trying next key...`
        );
        lastError = error;
        excludeKeyIndex = context.currentApiKeyIndex || -1;
        continue;
      }
  ...
  // 所有 key 都失败了
  fastify.log.error(`All ${maxRetries} API keys failed for provider ${provider.name}`);
